...
combs = pd.DataFrame(all_combination).T



#combs = pd.read_excel('brute_force_combi.xlsx')
...
for rule_num,rule in combs.iterrows():
	h_rsi = rule['h_rsi']
	adx = rule['adx']
	...
	for name in watchlist:
		...
		for index, row in data.iterrows(): 
			...
			buy_condition_7 = row.loc['adx'] > adx
	...
	# BUYING AN ALREADY "SELL" STOCK
		...
		if len(final) != 0:
			...
			#pprint(stock)
			print("\n")
			#pprint(f'ADX/RSI : {adx}/{h_rsi} ::The total profit and loss for {name} is {pnl_sum}')
			temp = str('ADX/RSI :') + str(adx) + '|' + str(h_rsi) + str('::The total profit and loss  ') + str(name) + str(' = ') + str(pnl_sum)
			summary.append(temp)
			#pdb.set_trace()
